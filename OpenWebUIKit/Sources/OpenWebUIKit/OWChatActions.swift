...
extension OpenWebUIClient {
    ...
    /// POST /api/tasks/chat/{id}/stop — cancels any in-flight generation for a chat.
    /// Called on Stop so the server doesn't keep generating past the point the user
    /// stopped and overwrite the truncated reply on reload.
    public func stopChatTasks(_ id: String) async throws {
        _ = try await send(request("/api/tasks/chat/\(encPath(id))/stop", method: "POST"))
    }
